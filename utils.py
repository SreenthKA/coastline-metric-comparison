...
class data_processor: 
    ...
    def get_rates(self, ref_img, img):
        """Calculate true positive, false positive, true negative, and false negative rates with zero division handling"""
        ...
        fp_fn_ratio = fp / (fn + epsilon)  # Prevent zero division
    ...
    def sobel_ed(self, img):
        sobel_x = cv2.Sobel(img, cv2.CV_64F, 1, 0, ksize=3)
        sobel_y = cv2.Sobel(img, cv2.CV_64F, 0, 1, ksize=3)
        sobel = cv2.magnitude(sobel_x, sobel_y)
        return cv2.convertScaleAbs(sobel)

    def prewitt_ed(self, img):
...
class data_visualizer:
    ...
    def plot_combined_metric_trends(self,metrics,ylabel=None,save_path=None):
        ...
        for j,metric in enumerate(metrics):
            ...
            if j == n - 1:
                # Set xticks with labels and font size
                ax[j].set_xticks(ticks=range(1, 13))
                ax[j].set_xticklabels(labels=self.channels, fontsize=15, rotation=90)
            else:
                # Set xticks without labels
                ax[j].set_xticks(ticks=range(1, 13))
                ax[j].set_xticklabels([])
    ...
    '''def get_title(self, ID, threshold, band=7):
        ...
        df = self.df_metrics[(self.df_metrics['ID'] == ID) & (self.df_metrics['band'] == band)].copy()

        # Convert thresholds to tuple format
        df['thresholds'] = df['thresholds'].apply(lambda x: tuple(ast.literal_eval(x)) if isinstance(x, str) else x)

        df_thresh = df[df['thresholds'] == tuple(threshold)]
        if df_thresh.empty:
            print(f"Warning: No matching threshold found for ID {ID} and threshold {threshold}")
    ...
    def get_title(self,ID,threshold,band = 7):
        ...
        #df['thresholds'] = df['thresholds'].apply(lambda x: tuple(ast.literal_eval(x)) if isinstance(x, str) else x)
        df.loc[:, 'thresholds'] = df['thresholds'].apply(lambda x: tuple(ast.literal_eval(x)) if isinstance(x, str) and x.startswith('(') else str(x))


        #df_thresh = df[df['thresholds']==threshold]
        df_thresh = df[df['thresholds'].astype(str) == str(threshold)]
        if df_thresh.empty:
            print(f"Warning: No matching threshold found for ID {ID} and threshold {threshold}")
            return "No matching data"
